...
@Controller
@RequestMapping("/salespfmc")
public class SalesPfmController {

	@Autowired
	private SalesPfmServiceimpl spservice;
	@RequestMapping("/uploadpfmtable.do")
	public ModelAndView uploadPfmTable(HttpServletRequest request, HttpServletResponse response) throws IllegalStateException, IOException {
		
		ModelAndView mav = new ModelAndView();
		
		System.out.println("开始销售业绩表上传");
		...
		if (multipartResolver.isMultipart(request)) {
			...
			while (iter.hasNext()) {
				...
				if (file != null) {
					...
					out.print("<script language=\"javascript\">alert('上传成功！');window.location.href='/out/login'</script>");
				*/
	...
	@ResponseBody
	 public ModelAndView exportExcel(ModelMap model, HttpServletRequest request, HttpServletResponse response){
		...
		XSSFWorkbook wbook = spservice.getDirExcel(path);
		XSSFWorkbook updwbook = spservice.updateExcel(wbook);
